...
while True:
    notif.led(BLUE)
    notif.oled_text("Temperature: {:0.1f} C\nHumidity: {:0.1f} %\nPressure: {:0.1f} hPa".format(bme280.temperature, bme280.humidity,bme280.pressure))
    buf = bytes("({:0.1f},{:0.1f},{:0.1f})\n".format(bme280.temperature, bme280.humidity,bme280.pressure), "utf8")
    serial.write(buf)
    notif.led(GREEN)
    time.sleep(1)