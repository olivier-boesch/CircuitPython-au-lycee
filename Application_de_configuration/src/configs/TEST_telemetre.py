...
# -------------- boucle (loop)
while True:
    # attente d'un appui sur le bouton C
    butC.check()
    ...
    with pulseio.PWMOut(broche_US, duty_cycle=2 ** 15) as pwm:
        with pulseio.PulseOut(pwm) as p:
            pulse_array = array.array('H',[2000])  # rester 2000µs à 3.3V puis 0v
            p.send(pulse_array)
    ...
    with pulseio.PulseIn(broche_US, maxlen=2) as p:
        ...
        while len(p) == 0:
            pass
        led.value = False  # on éteint la led -> fon de mesure
